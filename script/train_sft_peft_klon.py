...
# how to run
"""
python -m torch.distributed.launch --nproc_per_node=4 train_sft_peft_klon.py \
    --per_device_train_batch_size 1 \
    --gradient_accumulation_steps 32 \
    --num_train_epochs 10 \
    --wandb_project klon-xglm-7.5B_peft \
    --peft_model_id /workspace/ChomGPT/script/facebook/adapter-xglm-7.5B \
    --dataset_name pythainlp/klongklon \
    --adapter_name facebook/adapter-klon-xglm-7.5B
"""
...
#     per_device_eval_batch_size: Optional[int] = field(default=16)
    ...
    #model and dataset
    base_model_name_or_path: Optional[str] = field(default="facebook/xglm-1.7B")
    peft_model_id: Optional[str] = field(default="/workspace/ChomGPT/script/facebook/adapter-xglm-7.5B")
    dataset_name: Optional[str] = field(default="pythainlp/klongklon")
    ...
    #save stuff
    adapter_name: Optional[str] = field(default='facebook/adapter-klon-xglm-1.7B')
...
training_args = TrainingArguments(
    output_dir=f"{script_args.peft_model_id}_sft_peft_klon",
...
config = PeftConfig.from_pretrained(script_args.peft_model_id)
